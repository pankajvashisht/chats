...
class ChatController extends ApiController {
	...
	async lastChat(Request) {
		...
		const query = `select chats.*, users.id as friend_id,(select count(id) from chats  where is_read = 1 and receiver_id = ${user_id} and sender_id = users.id) as un_read_message, users.profile,users.user_type, users.phone,users.email, users.name, users.cover_pic, users.about_us
